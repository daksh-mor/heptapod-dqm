...
class CrossDepthMultiheadSelfAttention(nn.Module):
    def __init__(self, k_factor, in_channels):
        ...
        self.qkv_weight = nn.Parameter(torch.randn(in_channels, k_factor * 3, k_factor))
    ...
    def forward(self, x):
        ...
        qkv = torch.einsum('blck, ckm -> blcm', x_channels, self.qkv_weight.transpose(1, 2))
        qkv = qkv + self.qkv_bias.unsqueeze(0).unsqueeze(0)  # (batch_size, length, k_factor*3, in_channels)
        ...
        #q shape after: (batch_size, length, in_channels, 1, hidden_dim)
        #k shape after: (batch_size, length, 1, in_channels, hidden_dim)
...
class CrossDepthMultiheadCrossAttention(nn.Module):
    def __init__(self, k_factor, in_channels):
        super(CrossDepthMultiheadCrossAttention, self).__init__()
        self.k_factor = k_factor
        self.in_channels = in_channels


        self.q_weight = nn.Parameter(torch.randn(in_channels, k_factor, k_factor))
        self.q_bias = nn.Parameter(torch.randn(in_channels, k_factor))
        self.kv_weight = nn.Parameter(torch.randn(in_channels, k_factor * 2, k_factor))
        self.kv_bias = nn.Parameter(torch.randn(in_channels, k_factor * 2))
        self.fc_out = nn.Linear(k_factor, k_factor)
    ...
    def reset_parameters(self):
        # Initialize each channel's parameters using the same strategy as nn.Linear.
        for i in range(self.in_channels):
            nn.init.kaiming_uniform_(self.q_weight[i], a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.kv_weight[i], a=math.sqrt(5))
            fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.q_weight[i])
            bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
            nn.init.uniform_(self.q_bias[i], -bound, bound)
            nn.init.uniform_(self.kv_bias[i], -bound, bound)
    ...
    def forward(self, x, context):
        ...
        # Reshape to separate channels: (batch_size, length, in_channels, k_factor)
        x_channels = x.view(batch_size, length, self.in_channels, self.k_factor)
        context_channels = x.view(batch_size, length, self.in_channels, self.k_factor)


        # For each channel, multiply x (batch_size, length, k_factor) with weight (k_factor, k_factor)
        q = torch.einsum('blck, ckm -> blcm', x_channels, self.q_weight.transpose(1, 2))
        q = q + self.q_bias.unsqueeze(0).unsqueeze(0)  # (batch_size, length, in_channels, k_factor)

        # For each channel, multiply x (batch_size, length, k_factor) with weight (k_factor, k_factor * 2)
        kv = torch.einsum('blck, ckm -> blcm', context_channels, self.kv_weight.transpose(1, 2))
        kv = kv + self.kv_bias.unsqueeze(0).unsqueeze(0)  # (batch_size, length, in_channels, k_factor)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = CrossDepthMultiheadSelfAttention(k_factor, in_channels)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = CrossDepthMultiheadSelfAttention(k_factor, in_channels)
        self.ln_2 = norm_layer(hidden_dim)
        self.cross_attention = CrossDepthMultiheadCrossAttention(k_factor, in_channels)
        self.dropout = nn.Dropout(dropout)
