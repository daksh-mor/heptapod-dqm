...
data_path = os.path.abspath(os.path.join(os.path.dirname(current_path), "tools", "dqm", "test_files", "dataset"))
...
class DatasetTemplate(Dataset):
    ...
    def __init__(self, data_np_nd=None, **kwargs):
        ...
        self.timewindow_size = kwargs.get("memorysize", 5)
        ...
        print("{}\ndataset preparation...\n{}".format("#"*60, "#"*60))
        print("input data size ([ls x ieta x iph x depth x feature] or [ls x ieta x iph x feature]): {}".format(
            data_np_nd.shape))
        ...
        if not self.isdata_scaled:
            if self.istrain:
                ...
                if self.input_scaling_alg:
                    ...
                    elif self.input_scaling_alg == "custom":
                        self.input_scaler =MinMaxScalerCustom(feature_range=(0, 1), min_value=0, max_value=2567)
                        print(f"Using custom MinMaxScaler. Min value: {self.input_scaler.min}, Max value: {self.input_scaler.max}")
...
def prepare_model_temporal_torch_dataset(**kwargs):
    ...
    if not istrain:
        train_data_ref = util.load_pickle("{}/{}.pkl".format(data_path,
            trainnref_filename))
    ...
    util.save_pickle("{}/{}.pkl".format(data_path, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-is', '--input_scaling_alg', type=str,
                        choices=['minmax', "max", "custom", None],
                        default=None,
    ...
    model_data_ts = util.load_pickle("{}/{}.pkl".format(data_path,
            args["data_filename"]))
    ...
    # # visualize
    # ls_sel = 5
    # for d in range(model_data_ts.shape()[-2]):
    #     util.plot_2d_heatmap(model_data_ts.get_np()[0][ls_sel,0,...,d, 0], 
    ...
    # util.plot_hist3d_heatmap(model_data_ts.get_np()[0][ls_sel,0,...,:, 0].transpose((0, 2, 1)), 
...
#TRAIN
#for i in HE/Naive_Ensemble/he_train_dataset*; do python3 ../src/model_datasets.py -it -s he -d ts -m 1 -is custom -ds HE/Naive_Ensemble -np $i -td ${i}/train_iid; done
#TEST
